...
bool c_rage_bot::should_stop(bool shoot_check)
{
	...
	if (!g_utils->on_ground() && (g_ctx.cmd->buttons & in_speed))
		return true;
	...
	if (g_ctx.local->velocity().length(true) < 0.95f)
		return false;
...
bool c_rage_bot::auto_stop()
{
	...
	int max_speed = (int)g_movement->get_max_speed() * 0.33f;
