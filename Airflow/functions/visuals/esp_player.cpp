	...
	if (enemy_esp.elements & 64)
	{
		...
		if (info->cheat_id > 0)
		{
			...
			case 2:
				...
				break;
			case 3:
				info->objs[iter++] = esp_objects_t{ true, esp_right, color(0, 255, 163), 0.f, 0.f, xor_str("AIRFLOW OPEN"), font_pixel, false, info->dormant_alpha };
				break;
			case 4:
				info->objs[iter++] = esp_objects_t{ true, esp_right, color(171, 102, 79), 0.f, 0.f, xor_str("AIRFLOW OLD"), font_pixel, false, info->dormant_alpha };
				break;
			case 5:
				info->objs[iter++] = esp_objects_t{ true, esp_right, color(0, 255, 163), 0.f, 0.f, xor_str("WEAVE BETA"), font_pixel, false, info->dormant_alpha };
				break;
			}
		}
		if (flags.armorvalue)
